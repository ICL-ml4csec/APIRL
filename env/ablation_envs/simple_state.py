...
class APIMutateEnv(gym.Env):
    ...
    def __init__(self, action_space, request_seq, all_requests, logins=None, apikeys=None, test=0, cookies=None):
        super(APIMutateEnv, self).__init__()
        with open('./env/special_chars.txt') as chars:
            self.char_actions = chars.read().splitlines()
            
        self.action_space           = spaces.Discrete(action_space)
        self.observation_space      = spaces.Box(0, 3000, (4, 1), dtype=int)
    ...
    def _generate_state(self, value, status, endpoint=None):
        ...
        return np.array([var_type, request_type, status, param_location])
        ...
        if action_type == 'increase' and param_type is not None:
            ...
            if var == 'refresh':
                ...
                if len(self.logins) == 1:
                    if len(self.interface.apikey) > 0:
                    ...
                    elif len(self.interface.cookie) > 0:
            ...
            if self.parameter['name'] in self.dynamic_param_table.keys:
                ...
                for dynamic_param in self.dynamic_param_table.keys:
                    if re.search(self.parameter['name'], dynamic_param, re.I):
                        possible_vals = self.dynamic_param_table.store[dynamic_param].values
                        if len(possible_vals) > 0:
                            inj_var = random.choice(possible_vals)
    ...
    def  _compute_reward(self, req_param, action_type, inj_var):
        ...
        if 200 <= api_response.status_code <=  299:
            return 1, 0, api_response.status_code, api_response
        elif 300 <= api_response.status_code <=  499:
            return -1, 0, api_response.status_code, api_response
        else:
            return 10, 1, api_response.status_code, api_response
    ...
    def _create_payload(self, req_param):
        ...
        for param in req_param:
                ...
                if 'sub_type' in param.keys():
                    if re.search('int', param['sub_type']):
                        if random.choice([0, 1]) == 0:
                            ...
                            input_param = [1]
                    if re.search('str', param['sub_type']):
                        if random.choice([0, 1]) == 0 and re.search('register', self.current_request.path, re.I) is None:
                            input_param = ['']
                        else:
                            input_param = [''.join(random.choice(string.ascii_uppercase + string.digits) for _ in range(8))]
            ...
            if re.search('register', self.current_request.path) is None:
                    ...
                    try:
                        if dynamic_param_id is None:
                            if 'user' in self.dynamic_param_table.keys:
                                dynam_input_usr = list(self.login)[0]
    ...
    def _discrete_action_to_continuous(self, discrete_action):
        ...
        elif discrete_action in range(23, 28):
            type = ['put', 'patch', 'get', 'post', 'delete']
            action = ('request_method', type[discrete_action-23])
    ...
    def _parse_response(self, response, request_params):
        if response.text is not None and response.text != '':
            ...
            try:
                ...
                self.headers['Authorization'] = ''.join(['Bearer ', response_params[dynamic_param] ])
                self.interface.headers = self.headers
            except:
                try:
                    if type(response_params) == str and re.search('login|sigin|auth|token', response.url, re.I):
                        self.headers['Authorization'] = ''.join(['Bearer ', response_params])
                        self.interface.headers = self.headers
                except:
                    pass
                pass
        ...
        if user_login != None:
            try:
                ...
                if len(self.logins) > 0:
                    self._parse_response(response, request_params)
                else:
